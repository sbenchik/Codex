...
#
# The file was taken from eric 4.4.3 and adopted for codimension, then adopted
# for Codex.
# Original copyright:
...
class LexerPygments( LexerContainer ):
    ...
    def styleText( self, start, end ):
        ...
        text = unicode(config.m.edit.text() )[ :end + 1 ].encode( 'utf-8' )
        textLen = len( text )
        if self.__lexerGuessed == False:
            self.__lexer = self.__guessLexer()
        ...
        if self.__lexer is None:
            config.m.edit.setStyling( textLen, PYGMENTS_DEFAULT )
        else:
